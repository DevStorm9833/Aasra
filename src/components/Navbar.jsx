          ...
          <div className="hidden md:flex items-center gap-10">
            {navItems.map((item) => (
              <button 
                ...
                className={`text-[11px] font-bold uppercase tracking-[0.2em] transition-all relative py-1 ${activePage === item.id ? 'text-[var(--color-accent-orange)]' : 'text-[var(--color-gray-mid)] hover:text-[var(--color-primary-black)]'}`}
              >
                {item.name}
                {activePage === item.id && (
              ...
              <div className="flex flex-col gap-6 text-center">
                {navItems.map((item) => (
                  <button 
                    key={item.id} 
                    onClick={() => handleNavClick(item.id)} 
                    className={`text-sm font-bold uppercase tracking-widest ${activePage === item.id ? 'text-[var(--color-accent-orange)]' : 'text-[var(--color-gray-mid)]'}`}
                  >
                    {item.name}
                  </button>
                ))}
                ...
                <button className="w-full py-5 bg-gradient-to-r from-[var(--color-primary-black)] to-[#1E293B] text-[var(--color-primary-white)] text-xs font-bold uppercase tracking-[0.2em] rounded-full shadow-lg flex items-center justify-center gap-3">
